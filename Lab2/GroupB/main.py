...
class SoftmaxRegression:
    def __init__(self, input_size, num_classes, learning_rate=0.01, patience=10):
        self.W = np.zeros((input_size, num_classes))  # 權重矩陣
        self.b = np.zeros((1, num_classes))  # 偏差向量
    ...
    def forward(self, x):
        y_hat = np.dot(x, self.W) + self.b  # 線性變換 y = Wx + b
        probs = self.softmax(y_hat)  # 對 y 套用 softmax，得到每類機率 (0 ~ 1)
        return probs
    ...
    def gradient_descent_update(self, x, y, y_hat):
        ...
        grad_W = np.dot(x.T, delta) / m  # 計算 W 的梯度，並做平均
        grad_W += (self.reg_lambda) * self.W  # 加上 L2 正則化梯度
        grad_b = np.sum(delta, axis=0, keepdims=True)  # 計算 b 的梯度
        return grad_W, grad_b
    
    def update_parameters(self, grad_W, grad_b):
        self.W -= self.learning_rate * grad_W  # W = W - lr * grad_W
        self.b -= self.learning_rate * grad_b  # b = b - lr * grad_b
...
if __name__ == '__main__':
    ...
    model = SoftmaxRegression(input_size, len(CLASSES), 0.01)  # 建立模型
